...
export class ServiceRequestsController {
  ...
  @ApiOperation({
    ...
    description:
      'Cliente solicita execução de um serviço específico. Provider responderá com um orçamento.',
  })
  ...
  @ApiBearerAuth('access-token')
  @ApiOperation({
    summary: 'Listar os meus pedidos',
    description: 'Devolve os pedidos do cliente autenticado.',
  })
  @ApiResponse({ status: 200, description: 'Pedidos obtidos com sucesso.' })
  findMine(
    @CurrentUser() user: { id: string },
    @Query() query: FindServiceRequestsQueryDto,
  ) {
  ...
  @ApiOperation({
    ...
    description: 'Acessível ao cliente, ao provider do serviço e ao ADMIN.',
  })
  @ApiParam({ name: 'id' })
  ...
  @Patch(':id/cancel')
  @ApiBearerAuth('access-token')
  @ApiOperation({ summary: 'Cancelar o meu pedido' })
  @ApiParam({ name: 'id' })
